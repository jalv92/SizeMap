...
public class BookMirrorTests
{
    ...
    // Zombie-book guard: a post-reconnect re-announce Add burst must not stack levels past
    // the configured cap (the root cause of the observed bids=20/asks=20 symptom). The cap
    // is a safety bound, NOT the feed's depth — hence it is passed in, never assumed.
    [Fact]
    public void Add_beyond_MaxLevels_evicts_the_farthest_level()
    {
        var b = new BookMirror(0.25, TimeSpan.FromSeconds(10), maxLevels: 10);
        for (int i = 0; i < 10; i++)
    ...
    // Rithmic delivers 40+ levels per side where NT Brokerage/Continuum stops near 10.
    // The mirror must keep whatever the feed sends instead of silently discarding depth.
    [Fact]
    public void Deeper_feed_is_retained_not_truncated_at_ten()
    {
        var b = NewBook();
        for (int i = 0; i < 40; i++)
            b.ApplyDepth(Dep(Side.Bid, DepthOp.Add, i, 21000.00 - i * 0.25, 10, i));
        Assert.Equal(40, b.Levels(Side.Bid).Count);
    }

    // Updates and removes beyond level 10 must land, not be dropped for being out of range.
    [Fact]
    public void Update_beyond_level_ten_is_applied()
    {
        var b = NewBook();
        for (int i = 0; i < 40; i++)
            b.ApplyDepth(Dep(Side.Bid, DepthOp.Add, i, 21000.00 - i * 0.25, 10, i));
        b.ApplyDepth(Dep(Side.Bid, DepthOp.Update, 35, 21000.00 - 35 * 0.25, 777, 41));
        Assert.Equal(777, b.Levels(Side.Bid)[35].Volume);
    }
